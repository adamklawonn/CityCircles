...
def test(request, from_zl=DEFAULT_ZOOM, to_zl=DEFAULT_ZOOM, center_x=WIDTH/2, center_y=HEIGHT/2, from_origin_x=DEFAULT_VIEWALBE_ORIGIN_LL_X, from_origin_y = DEFAULT_VIEWALBE_ORIGIN_LL_Y):
    ...
    for model_tile in Tile.objects.filter(level=to_zl):
        ...
        if viewable_rect.overlaps(tile_rect):
            li_tiles.append(["/gtiles/" + model_tile.fl_name, n_tile_ll_w_x - n_ll_w_x, HEIGHT - (TILE_SIZE + (n_tile_ll_w_y - n_ll_w_y)) ])
            
    #NOW THE LIGHTRAIL LINES
    li_shapes = []
    
    model_pnts = SegmentPoint.objects.all()
    
    for model_pnt in model_pnts:
        ll_m_x, ll_m_y = mercator.LatLonToMeters(model_pnt.lat, model_pnt.lng)
        x, y = mercator.MetersToPixels(ll_m_x, ll_m_y, to_zl)
        
        
        li_shapes.append(["0", x - n_ll_w_x, HEIGHT - (y - n_ll_w_y)])
    ...
    dct_return["points"] = simplejson.dumps(li_shapes)
    dct_return["south_stations"] = []
    dct_return["north_stations"] = []
